...
export const gqlQuery = ()=>{
    if(!_gqlQuery) {
        _gqlQuery = gql`query cmsPage($slug:String!,$query:String,$props:String,$nosession:String,$editmode:Boolean,$dynamic:Boolean,$_version:String){cmsPage(slug:$slug,query:$query,props:$props,nosession:$nosession,editmode:$editmode,dynamic:$dynamic,_version:$_version){cacheKey slug name{${config.LANGUAGES.join(' ')}} urlSensitiv template script serverScript resources dataResolver ssr public online resolvedData style parseResolvedData alwaysLoadAssets ssrStyle compress html subscriptions _id modifiedAt createdBy{_id username} status}}`
    }
    return _gqlQuery
}
