...
export const getCmsPage = async ({db, context, slug, editmode, _version, headers}) => {
    ...
    if (!cmsPages) {
        ...
        cmsPages = await GenericResolver.entities(db, context, 'CmsPage', ['slug', 'name', 'template', 'script', 'serverScript', 'dataResolver', 'resources', 'ssr', 'public', 'urlSensitiv', 'parseResolvedData'], {
            match,
            limit: 1,
            _version
        })
