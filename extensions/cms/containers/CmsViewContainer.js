...
class CmsViewContainer extends React.Component {
    ...
    render() {
        ...
        console.info(`render ${this.constructor.name} editMode=${editMode} for ${slug} _id=${cmsPage._id} (loading=${this.props.loading}) in ${new Date() - startTime}ms / time since index.html loaded ${(new Date()).getTime() - _app_.start.getTime()}ms`)
        return content
    }
