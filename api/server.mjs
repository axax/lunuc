    ...
    dbConnection(MONGO_URL, (err, db, client) => dbPreparation(db, () => {
        ...
        if (!db) {
            ...
            app.post('/graphql', (req, res, next) => {
                ...
                // Check if the query is a "Probes" query and block it
                if (Array.isArray(dynamicConfig.BLOCK_GRAPHQL_QUERY) &&
                    dynamicConfig.BLOCK_GRAPHQL_QUERY.length > 0 &&
                    req.body && typeof req.body.query === 'string') {

                    const queryToCheck = req.body.query.trim(); // Leerzeichen am Anfang/Ende entfernen

                    // Überprüfe, ob die Query mit einem der konfigurierten Präfixe beginnt
                    const isBlocked = dynamicConfig.BLOCK_GRAPHQL_QUERY.some(prefix => {
                        return queryToCheck.startsWith(prefix);
                    });

                    if (isBlocked) {
                        console.warn('Blocked query matching a configured prefix from', req.ip);
                        res.writeHead(403, {'content-type': 'application/json'})
                        return res.end(JSON.stringify({
                            errors: [{
                                message: `Access to queries starting with one of the forbidden prefixes is blocked by configuration.`,
