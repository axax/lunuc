...
export const getFileFromOtherServer = async (urlPath, filename, baseResponse, req) => {
    ...
    // backup server
    console.log('load from backup server - ' + 'http://100.97.178.113/lunucserver/vmi2319914.contaboserver.net/webroot/uploads/'+filename.substring(13))
    const response = await downloadUrl('http://100.97.178.113/lunucserver/vmi2319914.contaboserver.net/webroot/uploads/'+filename.substring(13))
    if(!response.error && response.statusCode == 200) {
        const passStream = new PassThrough()
        response.pipe(passStream)
        passStream.pipe(baseResponse)
